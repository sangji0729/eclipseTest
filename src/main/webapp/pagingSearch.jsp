...
<button onclick="location.href='./${pageName}?page=1&search=${search }&searchName=${searchName}'">To the
	front</button>
<c:if test="${page lt 11 }">
	<button disabled="disabled">Previous</button>
</c:if>
<c:if test="${page gt PAGENUMBER }">
	<button onclick="location.href='./${pageName}?page=${page - 10}&search=${search }&searchName=${searchName}'">
		Previous</button>
</c:if>
...
<c:if test="${page ne 1 }">
	<button onclick="location.href='./${pageName}?page=${page - 1}&search=${search }&searchName=${searchName}'">◀</button>
</c:if>

<c:forEach begin="${startPage }" end="${endPage }" var="i">
	<c:if test="${i eq page }">
		<a href="./${pageName}?page=${i }&search=${search }&searchName=${searchName}"><button>${i }</button></a>
	</c:if>
	<c:if test="${i ne page }">
		<a href="./${pageName}?page=${i }&search=${search }&searchName=${searchName}">${i }</a>
	</c:if>
</c:forEach>
...
<c:if test="${page lt totalPage - 9 }">
	<button onclick="location.href='./${pageName}?page=${page + 10}&search=${search }&searchName=${searchName}'">Next</button>
</c:if>
<c:if test="${page gt totalPage - 10 }">
	<button disabled="disabled">Next</button>
</c:if>
<button onclick="location.href='./${pageName}?page=${totalPage }&search=${search }&searchName=${searchName}'">To
	the end</button>
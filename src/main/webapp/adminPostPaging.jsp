...
<button onclick="location.href='./${pageName}?category=${category }&page=1'"  >To the
	front</button>
...
<c:if test="${page gt PAGENUMBER }">
	<button onclick="location.href='./${pageName}?category=${category }&page=${page - 10}'">
		Previous</button>
</c:if>
...
<c:if test="${page ne 1 }">
	<button onclick="location.href='./${pageName}?category=${category }&page=${page - 1}'">◀</button>
</c:if>

<c:forEach begin="${startPage }" end="${endPage }" var="i">
	<c:if test="${i eq page }">
		<a href="./${pageName}?category=${category }&page=${i }"><button>${i }</button></a>
	</c:if>
	<c:if test="${i ne page }">
		<a href="./${pageName}?category=${category }&page=${i }">${i }</a>
	</c:if>
</c:forEach>
...
<c:if test="${page lt totalPage - 9 }">
	<button onclick="location.href='./${pageName}?category=${category }&page=${page + 10}'">Next</button>
</c:if>
<c:if test="${page gt totalPage - 10 }">
	<button disabled="disabled">Next</button>
</c:if>
<button name="toEnd" onclick="location.href='./${pageName}?category=${category }&page=${endPage }'">To
	the end</button>

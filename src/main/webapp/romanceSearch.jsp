...
<body>
	<c:set var="pageName" value="romanceSearch" scope="request" />
	...
	<div id="boardMain">
		...
		<div id="mainTopContainer">
			...
			<div id="searchBarBox">
				<form action="./romanceSearch" method="post" id="searchBar">
					<select name="searchColumn" id="searchColumn">
						<option value="rtitle"
							<c:out value="${searchColumn == 'rtitle'? 'selected':''}"/>>제목</option>
						<option value="rcontent"
							<c:out value="${searchColumn == 'rcontent'? 'selected':''}"/>>내용</option>
						<option value="name"
					...
					<c:forEach items="${list }" var="l">
						<tr>
							...
							<td class="articleTitle" colspan="8"><a
								href="romanceBoardDetail?rno=${l.rno }">${l.rtitle }</a></td>
						</tr>
						<tr>
							<td class="articleContent" colspan="8"><c:set var="content"
									value="${l.rcontent } " /> <c:if
						...
						<tr>
							...
							<td class="articleCount_Right"><span
								...
								class="valueText">${l.rcount }</span></td>
							...
							<td class="articleComment_Right"><span
								...
								class="valueText">${l.commentcount }</span></td>
							...
							<td class="articleLike_Right"><span class="articleLikeText">
									추천 </span>&nbsp;<span class="valueText">${l.rlike }</span></td>
							<td class="articleDate">${l.rdate }</td>
					...
					<tr>
                     ...
                     1
									...
									<c:otherwise>
										<c:if test="${page > 5}">
											<a
												href="./romanceSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=1"
												class="pageArrow"> <img class="leftArrow"
												src="./img/doublenext.png" alt="toStartPage" width="45px">
												<img class="leftArrow" src="./img/doublenext2.png"
												alt="toStartPage" width="45px">
											</a>
										</c:if>
										<c:if test="${startPage > 5}">
											<a
												href="./romanceSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${startPage - 5}"
												class="pageArrow"> <img class="leftArrow"
										...
										<c:forEach var="i" begin="${startPage }" end="${endPage }">
											<c:choose>
												...
												<c:otherwise>
													<a
														href="./romanceSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${i }"
														class="pageValue">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
                     ...
                     &nbsp;
											...
											<c:when
												...
												<a
													href="./romanceSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${totalPage }"
													class="pageArrow"> <img src="./img/doublenext.png"
													alt="toEndPage" width="45px"> <img
													src="./img/doublenext2.png" alt="toEndPage" width="45px">
												</a>
											</c:when>
										</c:choose>
									</c:otherwise>

...
<div id="headerSpace">
    ...
    <div id="boardMain">
        ...
        <div id="mainTopContainer">
            <div id="listPopular">
                            <c:forEach items="${bestList }" var="list2" >
                <div class="article top">
                    <div class="thumbnailBox">
                       <c:choose>
							<c:when test="${list2.dthumbnail ne null}">
                            <img class="thumbnail" src="./thumbnail/${list2.athumbnail}" alt="thumbnail">
                            </c:when>
							<c:otherwise>
                    ...
                    <div class="contentBox">
                        <span style="font-size:25px;"><b>${list2.dtitle }</b></span>
                        <br>
                      <c:set var="content" value="${list2.dcontent } "/>
                  	<c:if test="${fn:length(content) <= 130 }">
                  		<c:out value="${content}"/>
                  	</c:if>
                  	<c:if test="${fn:length(content) > 130 }">
                  		<c:out value="${fn:substring(content, 0, 130) }"/>
                  	</c:if>
                    </div>
                </div>
               </c:forEach>
			...
			<c:forEach items="${boardList.list }" var="list">
                    ...
                    <tr>
                        <td id="boardPage" colspan="9">
							...
							<c:otherwise>
							<c:if test="${pageNo > 5}">
                            <a href="./drama?pageNo=1" class="pageArrow">
                            ...
                            <a href="./drama?pageNo=${boardList.startPage - 5}" class="pageArrow">
                            ...
                            <c:forEach var="i" begin="${boardList.startPage}" end="${boardList.endPage}">
                            	<c:choose>
                            		...
                            		<c:otherwise>
                            		 <a href="./drama?pageNo=${i}" class="pageValue">${i}</a>
                            		</c:otherwise>
                            	</c:choose>
                            </c:forEach>
                            ...
                            <a href="./drama?pageNo=${boardList.totalPages}" class="pageArrow">

...
<body>
<c:set var="pageName" value="thrillerSearch" scope="request" />
...
<div id="headerSpace">
    ...
    <div id="boardMain">
        ...
        <div id="boardListTitle">
            <span class="boardListTitleText"> 스릴러 </span>
        </div>
        <div id="mainTopContainer">
            ...
            <c:forEach items="${list2 }" var="l2" >
                <div class="article top">
                    <div class="thumbnailBox">
                       <c:choose>
							<c:when test="${l.tthumbnail ne null}">
                            <img class="thumbnail" src="./thumbnail/${l.tthumbnail}" alt="thumbnail">
                            </c:when>
							<c:otherwise>
                    ...
                    <div class="contentBox">
                        <span style="font-size:25px;"><b>${l2.ttitle }</b></span>
                        <br>
           ...
           <!-- 검색창 -->
            <div id="searchBarBox">
                <form action="./thrillerSearch" method="post" id="searchBar">
				<select name="searchColumn" id="searchColumn">
					<option value="ttitle"
						<c:out value="${searchColumn == 'ttitle'? 'selected':''}"/>>제목</option>
					<option value="tcontent"
						<c:out value="${searchColumn == 'tcontent'? 'selected':''}"/>>내용</option>
					<option value="name"
            ...
            <c:forEach items="${list }" var="l" >
                    <tr>
                        ...
                        <td class="articleTitle" colspan="8">
                            <a href="thrillerBoardDetail?tno=${l.tno }">${l.ttitle }</a>
                        </td>
                    </tr>
                    <tr>
                        <td class="articleContent" colspan="8">
                          <c:set var="content" value="${l.tcontent } "/>
                  	<c:if test="${fn:length(content) <= 30 }">
                  		<c:out value="${content }"/>
                  	</c:if>
                  	<c:if test="${fn:length(content) > 30 }">
                    ...
                    <tr>
                        ...
                        <td class="articleCount_Right">
                            <span class="articleCountText"> 조회수 </span>&nbsp;<span class="valueText">${l.tcount }</span>
                        </td>
                        ...
                        <td class="articleLike_Right">
                             <span class="articleLikeText"> 추천 </span>&nbsp;<span class="valueText">${l.tlike }</span>
                        </td>
                    ...
                    <tr>
                        <td id="toBoardWrite" colspan="9">
                            <button id="WriteButton" onclick="location.href='./thrillerWrite.jsp'">리뷰 작성</button>
                        </td>
                    </tr>
                     <!-- 페이징 -->
                    <tr>
                     ...
                     <c:otherwise>
                     <c:if test="${page > 5}">
                            <a href="./thrillerearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=1" class="pageArrow">
                     ...
                     <c:if test="${startPage > 5}">
                            <a href="./thrillerSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${startPage - 5}" class="pageArrow">
                            ...
                            <c:forEach var="i" begin="${startPage }" end="${endPage }">
                               <c:choose>
                                  ...
                                  <c:otherwise>
                                   <a href="./thrillerSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${i }" class="pageValue">${i}</a>
                                  </c:otherwise>
                               </c:choose>
                            </c:forEach>
                     ...
                     <c:when test="${totalPage - page >= totalPage-(totalPage / 5) * 5}">
                            <a href="./thrillerSearch?searchColumn=${searchColumn }&searchWord=${searchWord }&page=${totalPage }" class="pageArrow">
